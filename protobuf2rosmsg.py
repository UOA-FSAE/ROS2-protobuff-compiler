...
def parse_protobuf(f_proto: TextIOWrapper, msg_dest: str, srv_dest: str):
    ...
    for line in f_proto:
        if not(message) and not(service):
            ...
            # print(groups.groups())
            # append to file
            msg_label = groups.group(1)
            msg_type = groups.group(2)
            msg_name = groups.group(3)
            msg_id = groups.group(4) # not needed for msg file
            msg_comment = groups.group(5)
            
            message_obj.add_field(msg_label, msg_type, msg_name, msg_id, msg_comment)
...
def main():
    ...
    # TODO clean relative to the .proto file
